...
$.widget('upgoer.upgoer', {
  ...
  _create: function() {
    ...
    this.words.split(' ').forEach(function (word) { 
      dictionary[preprocess(word)] = true; 
    });

    var options = this.options;
    this.options.input.keyup(function (e) {
      var input     = options.input;
      var spelling  = options.spelling;
      var chatCallback = options.chatCallback;

      checkSpelling(input, spelling, dictionary);

      if(e.keyCode === 13){
        if (chatCallback !== undefined) {
          chatCallback(input.val());
        }
...
function preprocess(word){
  return stemmer(word.toLowerCase().replace(/[^a-z]/g, ''));
}
...
function checkSpelling(input, spelling, dictionary){
